...
while sum((theta - theta_A).^2) > 0.00001  
    ...
    for i = 1:numI         
        for j = 1:numP
            exp_temp(i,j) = exp(theta(j,1) + alpha_hat * Z(i,j) +...,
    ...
    % Compute the likelihood of person i going to place j
    for i = 1:numI
        denom(i,1) = sum(exp_temp(i,:));
        for j = 1:numP
            ind_llh(i,j) = exp_temp(i,j) / denom(i,1); 
        end
    end
    s = sum(ind_llh) / numI; 
...
%%%%%%%%%%%%%%
% Second Stage : regress to acquire beta
%%%%%%%%%%%%%%
